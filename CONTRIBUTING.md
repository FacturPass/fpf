...
### Where the typed implementations legitimately differ

"Behaviorally identical" is about what the format requires, not about the mechanics
each language uses to get there. The untyped JS reference can check things a typed
...
empty `legal` produces `legal.country:` and `legal.name:` errors. The shared
contract is [`test-vectors.json`](test-vectors.json), and every implementation
satisfies it: each rejects every `decode_failures` entry and produces at least one
error for every `validate_failures` entry.
